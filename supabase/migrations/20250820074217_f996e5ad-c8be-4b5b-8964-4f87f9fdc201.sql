...
-- 刪除舊政策（如果存在）
DROP POLICY IF EXISTS "管理員可查看自己資料" ON public.admin_users;
DROP POLICY IF EXISTS "服務角色完全存取" ON public.admin_users;

-- 創建新政策
CREATE POLICY "admin_can_view_own_data" ON public.admin_users
    FOR SELECT 
